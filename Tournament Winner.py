...
"""The key points here is to use a current Team to record the team with biggest numbers.
"""
def tournamentWinner(competitions, results):
    currentTeam = ""
    points = {currentTeam: 0}
    for i, competition in enumerate(competitions):
        ...
        if winningTeam not in points:
            points[winningTeam] = 0
        points[winningTeam] += 3
        if points[winningTeam] > points[currentTeam]:
